...
Requirements: [`specs/2026-09-25-datasources.md`](specs/2026-09-25-datasources.md), section 能力探测. `internal/pkg/probe` takes an already-opened `dsconn.Conn` and reports, per data source type, which backup capabilities are usable. It has no knowledge of scheduling, persistence or HTTP; the datasource service (a later task) opens the connection, calls `probe.Run`, and is responsible for the "无法探测" (could not probe) state when opening the connection itself fails or times out.
