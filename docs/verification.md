...
| SOCKS5 proxy | `192.168.8.141:11080`, user `opsnap` | `serjs/go-socks5-proxy`, username/password auth |
...
- The real chain to verify for the datasources round: `SOCKS5 (192.168.8.141:11080) → ssh-jump (12222) → ssh-target` for server-file, and `SOCKS5 → mysql80` / `SOCKS5 → pg16` directly (mysql80/pg16 have no SSH in front of them).
